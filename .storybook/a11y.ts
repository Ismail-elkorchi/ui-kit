...
export const playA11y = async ({canvasElement}: {canvasElement: HTMLElement}) => runA11y(canvasElement);

const documentRuleIds = ['document-title', 'html-has-lang', 'landmark-one-main'];
...
async function runA11yDocument() {
  ...
  if (results.violations.length > 0) {
    throw new Error(`A11y document violations:\n${formatViolations(results.violations)}`);
  }
}
