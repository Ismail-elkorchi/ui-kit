...
const createRect = (width: number, height: number): DOMRect => new DOMRect(0, 0, width, height);
...
describe('uik-resizable-panels', () => {
  ...
  it('resizes with keyboard and emits detail', async () => {
    ...
    const handle = panels.shadowRoot?.querySelector<HTMLElement>('[part="handle"]');
    ...
    panels.addEventListener('resizable-panels-resize', event => {
      lastDetail = (event as CustomEvent<UikResizablePanelsResizeDetail>).detail;
    });
  ...
  it('emits pointer resize phases', async () => {
    ...
    const phases: string[] = [];
    panels.addEventListener('resizable-panels-resize', event => {
      phases.push((event as CustomEvent<UikResizablePanelsResizeDetail>).detail.phase);
    });

    handle.dispatchEvent(new PointerEvent('pointerdown', {button: 0, clientX: 200, pointerId: 1, bubbles: true}));
    handle.dispatchEvent(new PointerEvent('pointermove', {button: 0, clientX: 240, pointerId: 1, bubbles: true}));
    handle.dispatchEvent(new PointerEvent('pointerup', {button: 0, clientX: 240, pointerId: 1, bubbles: true}));
