...
describe("uik-radio-group", () => {
  ...
  it("moves selection with arrow keys", async () => {
    ...
    const firstInput =
      radios[0]?.shadowRoot?.querySelector<HTMLInputElement>("input");
    firstInput?.focus();
  ...
  it("syncs value when radios arrive after initial render", async () => {
    ...
    const defaultSlot =
      group.shadowRoot?.querySelector<HTMLSlotElement>("slot:not([name])");
  ...
  it("applies aria labeling and required validity rules", async () => {
    ...
    group.shadowRoot
      ...
      ?.dispatchEvent(new Event("slotchange"));
    group.shadowRoot
      ?.querySelector<HTMLSlotElement>('slot[name="error"]')
      ?.dispatchEvent(new Event("slotchange"));
    group.shadowRoot
    ...
    expect(updatedControl?.getAttribute("aria-labelledby")).toContain(
      "uik-radio-group",
    );
  });
  ...
  it("handles slotchange for error and default slots", async () => {
    ...
    const errorSlot =
      group.shadowRoot?.querySelector<HTMLSlotElement>('slot[name="error"]');
  ...
  it("ignores non-arrow key events", async () => {
    ...
    (
      group as unknown as { onKeyDown: (event: KeyboardEvent) => void }
  ...
  it("ignores non-element radio change targets", async () => {
    ...
    (
      group as unknown as { onRadioChange: (event: Event) => void }
    ).onRadioChange({
