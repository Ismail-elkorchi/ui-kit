...
export class UikNavRail extends LitElement {
  ...
  private emitSelect(item: UikNavRailItem) {
    ...
    this.dispatchEvent(
      new CustomEvent("nav-rail-select", {
        detail,
        bubbles: true,
        composed: true,
      }),
  ...
  private resolveDefaultFocusId() {
    const active = this.items.find(
      (item) => item.id === this.activeId && !item.isDisabled,
    );
  ...
  private focusItemById(id: string) {
    ...
    const button = root?.querySelector<HTMLElement>(
      `uik-button[data-nav-rail-id="${id}"]`,
    );
    button?.focus();
  }
  ...
  private renderItem(item: UikNavRailItem, focusId: string) {
    ...
    return html`
      <div part="item" class="item" data-item-id=${item.id}>
        ${isActive
          ? html`<div part="item-indicator" class="indicator"></div>`
          : nothing}
        <uik-button
          part="item-button"
  ...
  override render() {
    ...
    const ariaLabel =
      this.ariaLabelValue || (ariaLabelledby ? undefined : "Navigation rail");
