...
export class UikTreeView extends LitElement {
  ...
  private resolveSelectionState(
    ...
    if (leafIds.length === 0) return selectedSet.has(id) ? 'checked' : 'unchecked';
    const selectedCount = leafIds.filter(leafId => selectedSet.has(leafId)).length;
  ...
  private emitSelect(item: UikTreeViewItem, checked: boolean, selectionState: SelectionState) {
    ...
    this.dispatchEvent(new CustomEvent('tree-view-select', {detail, bubbles: true, composed: true}));
  }

  private emitOpen(item: UikTreeViewItem) {
    const detail: UikTreeViewOpenDetail = {id: item.id, item};
    this.dispatchEvent(new CustomEvent('tree-view-open', {detail, bubbles: true, composed: true}));
  }

  private emitToggle(item: UikTreeViewItem, open: boolean) {
    const detail: UikTreeViewToggleDetail = {id: item.id, open, item};
    this.dispatchEvent(new CustomEvent('tree-view-toggle', {detail, bubbles: true, composed: true}));
  }
  ...
  private toggleSelection(item: UikTreeViewItem) {
    ...
    const nextState = this.resolveSelectionState(treeIndex, selectedSet, item.id);
  ...
  private focusItem(id: string) {
    ...
    void this.updateComplete.then(() => {
      const element = this.renderRoot.querySelector<HTMLElement>(`[data-item-id="${id}"]`);
      element?.focus();
    });
  }
  ...
  private renderItem(
    ...
    const state = this.resolveSelectionState(treeIndex, selectedSet, dataItem.id);
