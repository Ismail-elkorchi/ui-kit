...
function oklchRawTransform(token) {
  ...
  if (value?.colorSpace !== 'oklch' || !Array.isArray(value.components)) return undefined;
...
function findOutputByName(outputs, filename) {
  return outputs.find(
    (output) => output.filename === filename || output.filename.endsWith(`/${filename}`) || output.filename.endsWith(`\\${filename}`)
  );
}

async function terrazzoBuildCssText(tokenFiles, filename, baseSelector, transform) {
  ...
  const outputs = normalizeOutputs(await terrazzoBuild(tokens, { sources, config }));
  const output = findOutputByName(outputs, filename);
  if (!output) {
    const names = outputs.map((entry) => entry.filename).join(', ');
    throw new Error(`Terrazzo build missing output ${filename}. Outputs: ${names}`);
  }
...
const tailwindRules = [
  ...
  },
  {
    namespace: 'radius',
    match: (key, type) =>
      type === 'dimension' && (key.startsWith('radius.') || key.endsWith('.radius') || key.includes('.radius.')),
    name: (key) => toKebab(stripPrefix(key, 'radius.')),
    value: (key) => `var(${getCssVarName(key)})`
  },
  {
    namespace: 'border-width',
    match: (key) => key.startsWith('border.width.') || key.includes('borderWidth') || key === 'separator.thickness',
    name: (key) => toKebab(stripPrefix(key, 'border.width.')),
    value: (key) => `var(${getCssVarName(key)})`
  },
  {
    namespace: 'outline-width',
    match: (key) => key.startsWith('outline.width.') || key === 'focus.ring.width' || key.includes('focusRingWidth'),
  ...
  {
    namespace: 'ease',
    match: (key) => key.startsWith('motion.ease.') || key.includes('transitionEase'),
    name: (key) => toKebab(stripPrefix(key, 'motion.ease.')),
    value: (key) => `var(${getCssVarName(key)})`
  },
  {
    namespace: 'duration',
    match: (key) => key.startsWith('motion.duration.') || key.includes('transitionDuration'),
  ...
  {
    namespace: 'opacity',
    match: (key, type) => type === 'number' && (key.startsWith('opacity.') || key.endsWith('.opacity')),
...
function buildTailwindStrictTheme(tokens) {
  const namespaces = [...new Set(tailwindRules.map((rule) => rule.namespace))].sort();
  const resetLines = namespaces.map((name) => `  --${name}-*: initial;`).join('\n');
...
function buildResolvedTokenTree(tokens, valuesById) {
  const output = {};
  const sortedTokens = Object.values(tokens).sort((a, b) => a.id.localeCompare(b.id, 'en'));
...
async function buildResolvedTokens() {
  ...
  const variants = [
    { name: 'light.comfortable', context: { theme: 'light', density: 'comfortable' } },
    { name: 'light.compact', context: { theme: 'light', density: 'compact' } },
    { name: 'dark.comfortable', context: { theme: 'dark', density: 'comfortable' } },
    { name: 'dark.compact', context: { theme: 'dark', density: 'compact' } }
  ];

  for (const variant of variants) {
    const sources = resolveSourcesForContext(resolver, resolverDir, variant.context);
...
async function buildCssOutputs() {
  const [foundationSources, semanticSources, componentSources] = await Promise.all([
    listTokenFiles('tokens/foundation'),
    listTokenFiles('tokens/semantic'),
    listTokenFiles('tokens/components')
  ]);

  const baseSources = [...foundationSources, ...semanticSources, ...componentSources];

  const themeLightSources = [...baseSources, path.join(pkgRoot, 'tokens/themes/light.json')];
  const themeDarkSources = [...baseSources, path.join(pkgRoot, 'tokens/themes/dark.json')];
  const densityCompactSources = [...baseSources, path.join(pkgRoot, 'tokens/density/compact.json')];
  ...
  const baselineMap = new Map([...foundationMap.entries(), ...themeBaseMap.entries()]);
  ...
  const foundationBlock = injectReducedMotion(buildLayerBlock(':root', foundationMap));
  const themeBaseBlock = buildLayerBlock(':where(:root, [data-uik-theme])', themeBaseMap);
  const lightBlock = buildLayerBlock(
    ":where(:root:not([data-uik-theme]), [data-uik-theme='light'])",
    lightDiffMap,
    { colorScheme: 'light' }
  );
  const darkBlock = buildLayerBlock(":where([data-uik-theme='dark'])", darkDiffMap, { colorScheme: 'dark' });
  const compactBlock = buildLayerBlock(":where([data-uik-density='compact'])", compactDiffMap);
  ...
  await writeFile(path.join(distDir, 'themes/uik-theme-base.css'), `${themeBaseBlock}\n`);
  await writeFile(
  ...
  await writeFile(path.join(distDir, 'themes/uik-density-compact.css'), `${compactBlock}\n`);
  ...
  await writeFile(path.join(distDir, 'uik-tailwind.compat.css'), `${tailwindCompat}\n`);
  await writeFile(path.join(distDir, 'uik-tailwind.strict.css'), `${tailwindStrict}\n`);
  await writeFile(path.join(distDir, 'index.css'), `${baseCss}\n\n${tailwindStrict}\n`);
}
