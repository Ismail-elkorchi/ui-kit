...
(globalThis as {CustomEvent?: typeof TestCustomEvent}).CustomEvent = TestCustomEvent as unknown as typeof CustomEvent;
...
describe('UikShellRouter', () => {
  it('tracks current route and emits navigation events', () => {
    ...
    expect(subscriber).toHaveBeenCalledWith({view: 'page2', subview: undefined});
    expect(windowListener).toHaveBeenCalled();

    const event = windowListener.mock.calls[0]?.[0] as CustomEvent<UikShellNavigationDetail> | undefined;
