...
const escapeTableCell = value => normalizeText(value).replace(/\|/g, '\\|').replace(/\n/g, ' ');
...
const normalizeFeatures = features => {
  ...
  const normalized = features.map(feature => {
    ...
    if (!id || !name || !category || !baseline) {
      throw new Error(`Feature entries must include id, name, category, baseline. Missing in ${id || '<unknown>'}.`);
    }
...
const run = async () => {
  ...
  await fs.writeFile(docsContentPath, `${renderMarkdown(normalizedFeatures)}\n`, 'utf8');
};
