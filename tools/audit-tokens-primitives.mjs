...
const referencedNotInTokens = [...primitiveRefs].filter(
  (name) => !tokenDefines.has(name),
);
const internalRefs = referencedNotInTokens.filter((name) =>
  primitiveDefines.has(name),
);
const unknownRefs = referencedNotInTokens.filter(
  (name) => !primitiveDefines.has(name),
);

const tokensUnusedByPrimitives = [...tokenDefines].filter(
  (name) => !primitiveRefs.has(name),
);
